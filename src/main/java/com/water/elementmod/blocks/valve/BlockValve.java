...
import com.water.elementmod.blocks.EMBlockBaseNoCreativeTab;
...
public class BlockValve extends EMBlockBaseNoCreativeTab implements ITileEntityProvider
{
	...
	public BlockValve(String name, Material material, SoundType soundtype, Float hardness, Float resistance, 
	...
	{
		return Item.getItemFromBlock(EMCoreBlocks.VALVE);
	}
	
	@Override
	public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state)
	{
		return new ItemStack(EMCoreBlocks.VALVE);
	...
	{
		if(state.getValue(USED) == false)
		{
			this.setState(true, worldIn, pos);
			TileEntityValve tileentity = (TileEntityValve)worldIn.getTileEntity(pos);
			if(!worldIn.isRemote) tileentity.activate(worldIn);
			if(worldIn.isRemote)
			{
				playerIn.sendMessage(new TextComponentTranslation("message.em.waterboss.valve_closed"));
			}
		}
		return true;
	}
	...
	public static void setState(boolean used, World worldIn, BlockPos pos)
	{
		...
		if(used) worldIn.setBlockState(pos, EMCoreBlocks.VALVE.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(USED, true), 3);
		else worldIn.setBlockState(pos, EMCoreBlocks.VALVE.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(USED, false), 3);
	}
