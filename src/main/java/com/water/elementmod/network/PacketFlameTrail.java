...
public class PacketFlameTrail implements IMessage
{
	...
	public PacketFlameTrail()
	{
		this.messageValid = false;
	}
	
	public PacketFlameTrail(Entity ent, World world, Double xCoord, Double yCoord, Double zCoord, Double xOffset, Double yOffset, Double zOffset, int radius)
	{
		this.ent = ent;
		this.world = world;
		this.xCoord = xCoord;
	...
	public static class Handler implements IMessageHandler<PacketFlameTrail, IMessage>
	{

		@Override
		public IMessage onMessage(PacketFlameTrail message, MessageContext ctx) {
		...
		void processMessage(PacketFlameTrail message, MessageContext ctx)
		{
			...
			for(double r = 0.6D; r <= message.radius; r += 0.45D)
			{
				for(float i = 0.0F; i < 360.0F; i += 150.0F)
				{
					...
					Minecraft.getMinecraft().world.spawnParticle(EnumParticleTypes.FLAME, finalX, message.yCoord + 0.15D, finalZ, message.xOffset, message.yOffset, message.zOffset);
				}
			}
		}
