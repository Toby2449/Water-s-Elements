...
public class PacketParticleCircleExplosionVanilla implements IMessage
{
	...
	public PacketParticleCircleExplosionVanilla()
	{
		this.messageValid = false;
	}
	
	public PacketParticleCircleExplosionVanilla(Entity ent, World world, Integer type, Double xCoord, Double yCoord, Double zCoord, Double xOffset, Double yOffset, Double zOffset, int radius, Integer height)
	{
		this.ent = ent;
		this.world = world;
		this.type = type;
	...
	public static class Handler implements IMessageHandler<PacketParticleCircleExplosionVanilla, IMessage>
	{

		@Override
		public IMessage onMessage(PacketParticleCircleExplosionVanilla message, MessageContext ctx) {
		...
		void processMessage(PacketParticleCircleExplosionVanilla message, MessageContext ctx)
		{
			...
			for(int m = 0; m < 20; m++)
			{
				for(double r = 0.6D; r <= message.radius; r += 0.45D)
				{
					for(float i = 0.0F; i < 360.0F; i += 15.0F)
					{
						...
						Minecraft.getMinecraft().world.spawnParticle(EnumParticleTypes.getParticleFromId(message.type), finalX, (message.yCoord + rand.nextInt(message.height) + rand.nextDouble()) + 0.15D, finalZ, 0.0D, 0.0D, 0.0D);
					}
				}
			}
		}
