...
public class PacketVoidKnightLargeCircle implements IMessage
{
	...
	public PacketVoidKnightLargeCircle()
	{
		this.messageValid = false;
	}
	
	public PacketVoidKnightLargeCircle(Entity ent, World world, Integer type, Double xCoord, Double yCoord, Double zCoord, Double xOffset, Double yOffset, Double zOffset, int radius)
	{
		this.ent = ent;
		this.world = world;
		this.type = type;
	...
	public static class Handler implements IMessageHandler<PacketVoidKnightLargeCircle, IMessage>
	{

		@Override
		public IMessage onMessage(PacketVoidKnightLargeCircle message, MessageContext ctx) {
		...
		void processMessage(PacketVoidKnightLargeCircle message, MessageContext ctx)
		{
			...
			for(double r = 0.6D; r <= message.radius; r += 0.45D)
			{
				for(float i = 0.0F; i < 360.0F; i += 15.0F)
				{
					...
					if(rand.nextInt(99) > 75) ParticleSpawner.spawnParticle(EnumCustomParticleTypes.getParticleFromId(message.type), finalX, message.yCoord + 0.15D, finalZ, message.xOffset, message.yOffset, message.zOffset);
				}
			}
		}
