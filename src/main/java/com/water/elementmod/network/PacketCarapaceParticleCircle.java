...
public class PacketCarapaceParticleCircle implements IMessage
{
	...
	public PacketCarapaceParticleCircle()
	{
		this.messageValid = false;
	}
	
	public PacketCarapaceParticleCircle(Entity ent, World world, Integer type, Double xCoord, Double yCoord, Double zCoord, Double xOffset, Double yOffset, Double zOffset, int radius)
	{
		this.ent = ent;
		this.world = world;
		this.type = type;
	...
	public static class Handler implements IMessageHandler<PacketCarapaceParticleCircle, IMessage>
	{

		@Override
		public IMessage onMessage(PacketCarapaceParticleCircle message, MessageContext ctx) {
		...
		void processMessage(PacketCarapaceParticleCircle message, MessageContext ctx)
		{
			...
			for(double r = 0.6D; r <= message.radius; r += 0.2D)
			{
				for(float i = 0.0F; i < 360.0F; i += 15.0F)
				{
					...
					ParticleSpawner.spawnParticle(EnumCustomParticleTypes.getParticleFromId(message.type), finalX, message.yCoord + 0.15D, finalZ, message.xOffset, message.yOffset, message.zOffset);
				}
			}
		}
