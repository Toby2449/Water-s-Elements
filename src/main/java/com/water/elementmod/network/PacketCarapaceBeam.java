...
public class PacketCarapaceBeam implements IMessage
{
	...
	public PacketCarapaceBeam()
	{
		this.messageValid = false;
	}
	
	public PacketCarapaceBeam(Entity ent, World world, Integer type, Double xCoord, Double yCoord, Double zCoord, Double xOffset, Double yOffset, Double zOffset, Integer radius)
	{
		this.ent = ent;
		this.world = world;
		this.type = type;
	...
	public static class Handler implements IMessageHandler<PacketCarapaceBeam, IMessage>
	{

		@Override
		public IMessage onMessage(PacketCarapaceBeam message, MessageContext ctx) {
		...
		void processMessage(PacketCarapaceBeam message, MessageContext ctx)
		{
			...
			for(double r = 0.6D; r <= message.radius; r += 0.45D)
			{
				for(float i = 0.0F; i < 360.0F; i += 15.0F)
				{
					...
					ParticleSpawner.spawnParticle(EnumCustomParticleTypes.getParticleFromId(message.type), finalX, message.yCoord + 6D, finalZ, 0.0D, -0.625D, 0.0D);
				}
			}
		}
